...
按用户明确授权，独立项目 `raw_sparse_mask_schedule_probe` 新增
`raw_sparse_mask_schedule_probe_v2_shared`。E1–E4、E8–E12 仅在 v2 执行层清除：GPU 1–6
允许共享、取消 idle/occupancy 检查、允许多 worker、取消 T0+11h cutoff、retry cap 和
v1 P0/P1 重跑门；GPU 0/7、外部进程不抢占、labels-after-fit、失败审计和 provenance 仍保留。
历史 v1 与 `SHARED_RESOURCE_MAIN` 工件不改写、不混入 v2 聚合。

启动前修复 v2 worker 的两个执行器问题：不再强行把子进程失败的 `audit_ok` 覆盖为 true，
并将 timeout/子进程异常落盘为失败后按可重试性处理；NaN、shape/hash、label-leakage、断言和
协议完整性错误保持 terminal `incomplete_compute`。focused v2 contract test 与原项目 tests
在启动前验证。

## 2026-08-18 corruption_objective_compatibility_probe E0–E4 terminal result

独立项目 `corruption_objective_compatibility_probe` 完成冻结的 E0、E1、E1b 与 E3；E1 为
`6 datasets × 3 arms × 3 paired seeds = 54/54` completed-valid，18 个 biological P0/P2
单元在当前 H0、budget、label SHA256 校验后复用已关闭 C2，36 个新 Clean/non-biological
单元在物理 GPU 6 完成。GPU 0/7 全程禁用，未抢占 GPU 1–5 外部任务。E1b 为 54/54 CPU
no-fit，E3 为 6/6 raw-X descriptive audit。

E1 复现了三个 biological development datasets 上的 P2-vs-P0 增益（Mouse_retina
`+0.394898`、Baron Human `+0.126069`、Campbell `+0.146883`），但 non-biological
cross-domain gate 为 `G1=0/3`，training amplification gate 为 `G2=1/3`。因此按预注册规则
终态为 `STOP_GENERAL_CORRUPTION`，E2 objective matrix 未启动。该结果不支持 P2 的 universal
corruption claim、raw-X support 因果解释、objective interaction 或 independent holdout
generalization；E1/E1b 使用 threshold-defined dense H0 proxy，E3 raw-X zero/nonzero 仅作
描述性审计。adaptive policy、matcher optimization、GAN/learned generator、new Gate、
architecture search、corruption-rate sweep 与 holdout 继续锁定。Claude review 四轮无 acquittal，
其中首轮为 `4/10 not ready`，其余为 timeout/tool-access failure；随后 same-family
result-to-claim 复核给出 `claim_supported: partial`，仅确认上述 development-panel bounded
claim，不扩大到跨域、objective、raw-X 或 holdout。
