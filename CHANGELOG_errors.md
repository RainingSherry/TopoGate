...
在临时 GitHub release clone 中运行新项目 focused tests 时，`test_existing_run_reuse_rechecks_current_source_hashes`
直接依赖本地结果盘的 S0 `H0.npy`，而公开 clone 按发布边界不包含该输入，导致 collection 后单测
以 `FileNotFoundError` 失败；这不是 C2 结果失败。将测试改为 monkeypatch 合成 source/label
hash，仅验证 reuse 的 current-hash rejection contract，不改变 runner 或性能工件。随后本地与
