...
`sparse_corruption_principle_probe`，不使用 V 系列。问题收缩为 support/value/difficulty/geometry
机制定位与有限 static library；旧项目全部只读。C0 冻结三项 development roles、六个 primary
principles、exact budget、labels-after-fit-only firewall 和 GPU deny list。14 个可审计候选按
label-free maximin 选出 12 个 holdout sources（membership frozen before C2 results，runs
locked）。C1 已完成 54 个 zero-fit H0 structural replays；C2 随后完成 `54/54` 个正式 GPU
runs，并通过独立 `audit_ok=true` 审计。C2 的终态标签为
`simple_static_principle_sufficient`：P2_SupportTarget 在三个 development datasets 上均达到
`Delta_ARI >= 0.03`，P3/P4 各在两个数据集上达到该描述性阈值。C3 holdout、adaptive policy、
GAN 与 learned generator 仍锁定；C2 是 bounded development-panel evidence，不是泛化证据。
...
这是独立的表示/腐化机制验证项目，不是 V26。M0 已通过 exact C2 P2 replay 与 dormant holdout
freeze audit。M1 只授权一个 `P2_MM_SupportPreserve` control，但在 GPU 前的全 30-epoch
magnitude preflight 中，Baron Human 三个 seed 均超过冻结的 5% dataset-total L1 tolerance；
因此状态为 `magnitude_match_not_estimable`，没有启动任何 M1 GPU run。该状态不是性能 No-Go，
而是当前 active-active magnitude-matched control 无法在三数据集面板上形成完整 estimand；
M2 raw-X bridge、M3 holdout、M4 full backbone、adaptive policy 与 GAN 保持 locked。
