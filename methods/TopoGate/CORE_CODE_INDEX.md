...
| `V13_hard_gate/` | V13 | Gumbel-Top-k hard topology gate 与 masked reconstruction | `run_npz.py:main`、`train_and_evaluate` | V12 gate-selection successor |
| `V15_counterfactual_gate/` | V15 | counterfactual feature risk、稀疏 gate 与 paired readout | `run.py:run_one`、`trainer.py:fit_predict` | 独立探索分支 |
| `V16_predictive_graph_gate/` | V16 | predictive graph gate 与 sparse support diagnostics | `run.py`、`trainer.py` | 独立探索分支 |
| `V16_1_predictive_graph_gate/` | V16.1 | cross-fitted predictive graph、candidate assignment 与 sparse-domain checks | `run.py`、`trainer.py` | 独立扩展分支 |
...
| `V18_scmae_latent_gate/` | V18 | scMAE warmup、稀疏候选关系、latent gate 与 spectral/Leiden readout | `run.py:run_one`、`model.py:fit_v18` | 独立 relation-native 分支 |
| `V19_rg_adapter/` | V19 | reliability-gated NeighborMix-scMAE adapter | `run.py:run_one`、`trainer.py:fit_predict` | RG reproduction/extension adapter |
| `V20_topology_conditioned_adv_mask/` | V20 | topology-conditioned adversarial feature masking | `run.py:run_one`、`trainer.py:fit_full` | V21 前身，保留作对照 |
| `V21_assignment_adversarial_gate/` | V21 | bounded assignment divergence、Student-t surrogate 与 topology Gate | `run.py:run_one`、`trainer.py:fit_v21` | assignment-level adversarial branch |
| `V22_topology_discriminator_hard_mask/` | V22 | coordinate discriminator、topology hard mask 与 cooperative Keep-Gate | `run.py:run_one`、`trainer.py:fit_v22` | 当前 exploratory discriminator branch |
...
### V18：`V18_scmae_latent_gate/`

- `input_adapter.py`：NPZ/CSR 输入适配、无标签边界和稀疏 profile。
- `scmae.py`、`model.py`：scMAE warmup 与 latent relation fitting。
- `graph.py`、`relation.py`、`spectral.py`：候选关系、稀疏系数/affinity 和最终读出。
- `run.py`：单任务 CLI，`scripts/V18/` 提供 manifest、矩阵、审计和汇总入口。

### V19：`V19_rg_adapter/`

- `graph.py`：PCA-cosine kNN、mutual/SNN 和 edge reliability。
- `mixing.py`：reliability-weighted NeighborMix；`model.py` 复用 scMAE backbone。
- `trainer.py`、`run.py`：label-free fit API、外层 K 协议和完整输出契约。
- `scripts/V19/`：固定 manifest、X-only tuning、扩展矩阵和审计工具。

### V20：`V20_topology_conditioned_adv_mask/`

- `input_adapter.py`、`graph.py`：稀疏输入保留与 topology statistics。
- `model.py`：feature Gate 和 masked autoencoder。
- `trainer.py`：scMAE-only 与 topology-adversarial 训练路径；`scripts/V20/` 管理批量运行。

### V21：`V21_assignment_adversarial_gate/`

- `model.py`：feature Gate、Student-t cluster head 和 V21 autoencoder。
- `trainer.py`：随机 scMAE、assignment consistency、InfoMax 和 topology adversarial Gate。
- `readout.py`：clean embedding KMeans primary readout 与 Student-t diagnostic readout。
- `run.py`、`scripts/V21/`：显式 K、GPU 隔离、扩展矩阵和 provenance audit。

### V22：`V22_topology_discriminator_hard_mask/`

- `model.py`：scMAE generator、coordinate-matched discriminator 和 topology Gate。
- `graph.py`、`input_adapter.py`：四维 topology statistics、稀疏支持和 donor eligibility。
- `trainer.py`：random/scMAE、hard-negative Gate、reconstruction-hard Gate、non-topology Gate
  和 cooperative Keep-Gate；诊断记录 D shortcut 与 Gate gradient split。
- `run.py`、`scripts/V22/`：单任务入口、固定数据集 manifest、资源恢复和汇总工具。
- V22 的 cooperative Full 仍是单 seed 工程/资源审计，不能据此宣称跨数据集收益。
