...
from .config import INPUT_PROTOCOLS, VARIANTS, V0_RGConfig, load_config
from .input_adapter import encode_labels, load_matrix, prepare_input
...
def run_one(
    ...
    *,
    ...
    if not launcher_log.exists():
        launcher_log.write_text("direct V0_RG invocation; stdout was not captured\n", encoding="utf-8")
    return summary


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TopoGate V0_RG RG-NeighborMix-scMAE NPZ adapter")
    parser.add_argument("--data-path", required=True)
    parser.add_argument("--labels-path", default=None, help="optional one-label-per-row text file")
