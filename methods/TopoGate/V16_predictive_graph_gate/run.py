...
from .gate import assignment_readout, predictive_support, summarize_gate
from .graph import build_candidate_graph, candidate_recurrence
from .sparse import TheoryDomainError, load_npz_matrix, prepare_counts, repeated_splits
from .trainer import resolve_device, train_stage_a
...
def write_domain_status(
    *,
    ...
    """Persist a blocked theory-domain run without starting model fitting."""
...
def fit_v16(
    ...
    *,
    ...
    """Fit V16 without allowing labels into any model or graph operation."""
    config = config or V16Config()
    ...
    views = repeated_splits(prepared.counts, config.thinning_fraction, config.support_repeats, config.seed)
    graphs = [build_candidate_graph(view[0], config.graph_k) for view in views]
    graph = graphs[0]
    support, support_profile = predictive_support(
    ...
    summary: dict[str, Any] = {
        ...
        "device": str(device),
        "environment": {"python": platform.python_version(), "numpy": np.__version__, "torch": torch.__version__, "sklearn": sklearn.__version__},
        "output_files": {
...
def _load_npz(path: Path) -> tuple[np.ndarray | sp.csr_matrix, np.ndarray | None, str]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TopoGate V16 predictive graph gate")
    ...
    overrides = {k: v for k, v in {"variant": args.variant, "epochs": args.epochs, "batch_size": args.batch_size, "no_cuda": True if args.no_cuda else None}.items() if v is not None}
