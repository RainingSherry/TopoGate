...
def prepare_dual_input(
    ...
    *,
    ...
    ``scRNA_count`` uses the same frozen dual-view transform as the existing
    bridge protocols; accepting it here only makes the V25 holdout adapter
    explicit and does not alter either legacy protocol's behavior.
    """
    if input_protocol not in {"clubench_bridge", "shared_text", "scRNA_count"}:
        raise ValueError("V21 input protocol must be clubench_bridge, shared_text, or scRNA_count")
