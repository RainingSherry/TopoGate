...
```bash
python scripts/V26/run_matrix.py --all
```
...
This public release replaces the formal run's local input paths with relative
`datasets/` locations. Supply files matching the names and fields in
`protocol.py`; neither inputs nor formal model outputs are included. The
formal result and its original implementation digest are in
`reports/V26_support_oracle/`.
