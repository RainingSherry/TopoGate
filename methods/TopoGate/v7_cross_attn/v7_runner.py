"""v7 runner — isolated TopoGate training loop with cross-attention latent mix.

This module is a structural twin of `methods/TopoGate/v6_latent_mix/v6_runner.py`
that replaces the linear latent mix with a multi-neighbor cross-attention
residual mix.  **Only three changes vs v6**:

  1. neighbours are NOT averaged to one vector — each is encoded separately.
  2. B is NOT masked — the full neighbour fingerprint is fed to attention.
  3. mix step is `z_combined = z_a + gate * attn(z_a, Z_B)` instead of
     `z_mixed = (1-gate) * z_a + gate * z_n_mean`.

All other machinery (LearnableGate, compute_edge_reliability, schedule, MAE,
KMeans, metrics) is reused unchanged.  This isolates the experiment to
"how do neighbours participate" with everything else held constant.

设计契约
--------
- 不修改 methods/TopoGate/learnable_gate/ 任何文件
- 不修改 methods/TopoGate/v6_latent_mix/ 任何文件
- 不修改 baseline/ 任何文件
- 接口与 v6_runner.run_v6(**kwargs) 严格对齐

Reused
------
- AutoEncoder (model.encoder/mask_predictor/decoder)
- model.loss_mask_weighted, MicroMAEEncoder.encode/decode_from_latent
- LearnableGate, build_gate_stats_tensor
- compute_edge_reliability, build_*_neighbors
- compute_node_gate (for static gate fallback during warmup)

New
---
- CrossAttnMixer (cross_attn_mixer.py)
- v7_padded_neighbour_index(): returns (bsz, k) int64 with padding=0 fallback.
"""
...
from methods.TopoGate.v7_cross_attn.cross_attn_mixer import CrossAttnMixer  # noqa: E402
...
def parse_v7_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TopoGate v7 (cross-attn latent mix)")
    # Core (mirror v6_runner)
    parser.add_argument("--data_path", required=True)
    parser.add_argument("--save_dir", required=True)
    parser.add_argument("--dataset_name", default=None)
    parser.add_argument("--method_name", default="TopoGate")
    parser.add_argument("--variant_name", default="v7_cross_attn")
    ...
    parser.add_argument("--edge_reliability_mode",
                        default="sim_mutual_snn_distance")
    ...
    # LearnableGate init mode (parity with v6)
    ...
    # v7-specific
    parser.add_argument("--mask_b_anchor", type=str2bool, default=False,
                        help="v7-patch: B 不 mask（False=不 mask，让 attention 看到完整邻居指纹）")
    parser.add_argument("--mask_b_ratio", type=float, default=0.0)
    parser.add_argument("--cross_attn_heads", type=int, default=1)
    parser.add_argument("--cross_attn_residual_dropout", type=float, default=0.0)
...
# ──────────────────────────────────────────────
#  Masked noise (parity with v6/run_npz)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
#  Neighbour sampling (parity with v6)
# ──────────────────────────────────────────────
...
def train_and_evaluate(args: argparse.Namespace) -> Dict:
    """Train TopoGate with v7 cross-attn mix and return summary dict."""
    ...
    # Load data (parity with v6)
    X_raw, y_raw = load_data(args.data_path)
    dataset_name = args.dataset_name or Path(args.data_path).stem

    # Preprocess (parity with v6)
    ...
    print(
        f"[{dataset_name}] v7_cross_attn  device={device}  "
        f"n={X_np.shape[0]}  d={X_np.shape[1]}  K={n_clusters}",
        flush=True,
    )

    # Topology (parity with v6)
    ...
    # Per-node stats tensor (parity with v6)
    ...
    # CrossAttnMixer (v7 new — replaces LatentMixer)
    ...
    cross_mixer = CrossAttnMixer(
        ...
        enhanced_stats=int(args.enhanced_stats),
        attn_heads=int(args.cross_attn_heads),
        residual_dropout=float(args.cross_attn_residual_dropout),
    ).to(device)

    # Pre-compute v1-style static gate for schedule fallback (parity with v6)
    ...
    # Training loop (parity with v6 except pseudo branch)
    t0 = time.time()
    for epoch in range(1, args.epochs + 1):
        # Schedule t (parity with v6)
        if pseudo_enabled:
        ...
        for idx_t, x in train_loader:
            ...
            # ── Pseudo branch: cross-attn mix (v7) ──
            pseudo_loss = torch.zeros((), dtype=real_loss.dtype, device=device)
            if pseudo_enabled:
                # 1. Sample neighbours (numpy) — parity with v6
                ...
                if sampled_idx.shape[1] == 0:
                    ...
                    # ===== v7 vs v6 改动 1 =====
                    # 不取邻居 input 均值——逐个邻居独立 encode
                    # sampled_idx: (bsz, m=int(mix_neighbors))
                    bsz = x.shape[0]
                    m = sampled_idx.shape[1]
                    # 直接取所有 m 邻居 latent 形状 (bsz, m, num_genes)
                    x_neighbors_flat = X_np[sampled_idx]  # (bsz, m, num_genes)

                    # 把每个邻居 reshape 成 (bsz*m, num_genes) 编码
                    x_neighbors_t = torch.as_tensor(
                        x_neighbors_flat, dtype=x.dtype, device=device
                    )
                    x_neighbors_t = x_neighbors_t.view(bsz * m, -1)  # (bsz*m, num_genes)

                    # ===== v7 vs v6 改动 2 =====
                    # B 不 mask（默认 mask_b_anchor=False）；保留完整邻居指纹
                    if args.mask_b_anchor:
                        x_n_corrupt, _ = apply_mask_noise(x_neighbors_t, args.mask_b_ratio)
                    else:
                        x_n_corrupt = x_neighbors_t

                    # anchor 仍然 mask
                    z_anchor, _, _ = model.forward_mask(x_corrupt)  # (bsz, hidden)

                    # 每个邻居独立 encode → (bsz*m, hidden) → reshape (bsz, m, hidden)
                    z_neighbors_flat = micro_encoder.encode(x_n_corrupt)  # (bsz*m, hidden)
                    z_neighbors_padded = z_neighbors_flat.view(bsz, m, -1)  # (bsz, m, hidden)

                    # ===== v7 vs v6 改动 3 =====
                    # Cross-Attention Residual mix
                    batch_stats = stats_tensor[idx_t]
                    if t < 1.0:
                        sg = static_gate_tensor[idx_t]
                        z_combined, mix_info = cross_mixer(
                            z_anchor, z_neighbors_padded, batch_stats,
                    ...
                    # decode 回到输入空间
                    recon = micro_encoder.decode_from_latent(z_combined)  # (bsz, hidden_dim)

                    # MAE reconstruction loss with real_mask weighting (parity with v6)
            ...
            loss.backward()
            # freeze MAE params if asked (parity with v6)
            if mae_frozen:
        ...
        # beta history (every epoch, schedule + mae_frozen) — parity with v6
        beta_history.append({
            "epoch": epoch,
            "schedule_t": float(t),
            "mae_frozen": bool(mae_frozen),
            **cross_mixer.beta_snapshot(),
        })
    ...
    # Embedding extraction (parity with v6)
    model.eval()
    embeddings = []
    with torch.no_grad():
    ...
    summary = {
        ...
        "learned_gate_final_beta": cross_mixer.beta_snapshot(),
        "learned_gate_beta_history": beta_history,
        "v7_specific": {
            "mask_b_anchor": bool(args.mask_b_anchor),
            "cross_attn_heads": int(args.cross_attn_heads),
            "cross_attn_residual_dropout": float(args.cross_attn_residual_dropout),
        },
    }
    save_json(summary, str(save_dir / "summary.json"))

    print(
        f"[{dataset_name}] v7 done  {metrics_str}  time={train_time:.1f}s",
        flush=True,
    )
    return summary
...
# ──────────────────────────────────────────────
#  High-level wrapper (parity with run_v6)
# ──────────────────────────────────────────────

def run_v7(X, n_clusters, y=None, gpu=4, variant="v7_cross_attn", save_dir=None,
           seed=42, return_metrics=False, **overrides):
    """Top-level v7 entry point. Mirrors run_v6 for direct comparison."""
    import tempfile
    import shutil
    import io
    import contextlib
    ...
    if save_dir is None:
        save_dir = tempfile.mkdtemp(prefix=f"topogate_v7_{variant}_")
        _cleanup_tmp = True
    else:
    ...
    try:
        ...
        with contextlib.redirect_stdout(buf):
            summary = train_and_evaluate(parse_v7_args())
