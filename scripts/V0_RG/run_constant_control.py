#!/usr/bin/env python3
"""Run a fixed constant-gate V0_RG control on one manifest dataset.

This is deliberately separate from the frozen main tuning entry point. Labels
are read only after prediction for test scoring; preprocessing and fitting see X.
"""
...
def main() -> None:
    ...
    cfg=V0_RGConfig(protocol_id="topogate_constant_control_v1",variant="rg_full",epochs=80,gate_center=None,gate_adaptivity=None,gate_min=.1,gate_max=.1,neighbor_k=5,mix_neighbors=4,edge_reliability_mode="none",pseudo_weight=0.0)
    a.output.mkdir(parents=True,exist_ok=True); (a.output/"protocol.json").write_text(json.dumps({"dataset_id":a.dataset_id,"control":"constant_gate","gate_min":.1,"gate_max":.1,"seeds":list(SEEDS),"split_seed":9102026,"fit_scope":"train_validation","score_scope":"test","labels_used_during_fit":False},indent=2))
    rows=[]
    for seed in SEEDS:
        started=time.time(); pred,emb,diag=fit_predict(test_x,fit_X=fit_x,n_clusters=int(row["n_clusters"]),config=cfg,seed=seed,device=a.device); rows.append({"dataset_id":a.dataset_id,"method":"constant_gate","seed":seed,"ari":float(adjusted_rand_score(y[te],pred)),"nmi":float(normalized_mutual_info_score(y[te],pred)),"wall_seconds":time.time()-started}); np.save(a.output/f"predictions_seed_{seed}.npy",pred)
    import csv
