...
from methods.TopoGate.V16_1_predictive_graph_gate.config import V16_1Config, load_config
from methods.TopoGate.V16_1_predictive_graph_gate.gate import assignment_readout, summarize_gate
from methods.TopoGate.V16_1_predictive_graph_gate.run import (
    fit_v16_1,
    write_domain_status,
...
from methods.TopoGate.V16_1_predictive_graph_gate.trainer import DeviceUnavailableError
from scripts.V16_1.stress import apply_compound_stress
...
def run_one(
    ...
    *,
    ...
    dataset = path.stem
    metadata = resolve_metadata(dataset, registry)
    semantics = metadata.get("count_semantics")
    semantics_source = metadata.get("semantics_source")
    if isinstance(X, DenseNPZReference):
        ...
        for variant in VARIANTS:
            ...
            write_domain_status(
                save_dir=output_root / dataset / condition / variant / f"seed{seed}",
                ...
                run_metadata={
                    ...
                    "status_reason": "dense_input_not_supported",
                },
            )
        return
    ...
    reference_dir = condition_root / "V16_1_predictive_gate" / f"seed{seed}"
    try:
        _, base_summary = fit_v16_1(
            X,
    ...
    for variant in VARIANTS:
        if variant == "V16_1_predictive_gate":
            continue
        _write_readout(
            reference_dir,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="V16.1 paired readouts with one Stage-A/graph fit per seed")
    parser.add_argument("--data-root", default="datasets")
    parser.add_argument("--datasets", nargs="+", required=True)
    parser.add_argument("--save-root", default="result/V16_1/v16_1_paired")
    ...
    parser.add_argument("--expanded-count", action="store_true")
    args = parser.parse_args()
    overrides = {"variant": "V16_1_predictive_gate"}
    if args.expanded_count:
        overrides["input_policy"] = "expanded_count"
    if args.no_cuda:
        overrides["no_cuda"] = True
