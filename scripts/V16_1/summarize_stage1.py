    ...
    for dataset, conditions in sorted(grouped.items()):
        ...
        if first_row.get("status") == "theory_domain_not_supported" or certificate.get("theory_domain") != "candidate":
            output.append({
                ...
                "reason": ";".join(certificate.get("domain_reasons", [])),
                "domain_tier": certificate.get("domain_tier"),
            })
            continue
        ...
        complete = (
            len(clean_seeds) >= 3
            and len(stress_seeds) >= 3
            and all(clean_metrics.get(name) for name in NAMES)
            and all(stress_metrics.get(name) for name in NAMES)
        )
        if not complete:
            status = "incomplete"
            ...
            clean_delta = [a - b for a, b in zip(clean_metrics["V16_1_predictive_gate"], clean_metrics["self_only"])]
            clean_fixed = [a - b for a, b in zip(clean_metrics["V16_1_predictive_gate"], clean_metrics["fixed_predictive_graph"])]
            clean_shuffle = [a - b for a, b in zip(clean_metrics["V16_1_predictive_gate"], clean_metrics["shuffled_support"])]
            clean_output = [a - b for a, b in zip(clean_metrics["V16_1_predictive_gate"], clean_metrics["output_disabled"])]
            stress_delta = [a - b for a, b in zip(stress_metrics["V16_1_predictive_gate"], stress_metrics["self_only"])]
            ...
            passed = (
                ...
                and stress_retention is not None
                and stress_retention >= 0.50
            )
        ...
        output.append({
            ...
            "reason": reason,
            "domain_tier": certificate.get("domain_tier"),
            "clean_self_ari_mean": _mean(clean_metrics.get("self_only", [])),
            "clean_fixed_ari_mean": _mean(clean_metrics.get("fixed_predictive_graph", [])),
            "clean_v16_1_ari_mean": _mean(clean_metrics.get("V16_1_predictive_gate", [])),
            "clean_shuffled_ari_mean": _mean(clean_metrics.get("shuffled_support", [])),
            "clean_output_disabled_ari_mean": _mean(clean_metrics.get("output_disabled", [])),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="V16.1 fixed Stage-1 promotion summary")
