...
MPL_CACHE = str(Path(tempfile.gettempdir()) / "v26-mpl")
...
def run_blocking(command: list[str], log_path: Path) -> int:
    ...
    environment.update({"OPENBLAS_NUM_THREADS": "1", "OMP_NUM_THREADS": "1", "MKL_NUM_THREADS": "1", "NUMEXPR_NUM_THREADS": "1", "MPLCONFIGDIR": MPL_CACHE})
    ...
    while pending or active:
        ...
        if pending:
            ...
            for index, job in enumerate(pending):
                ...
                log_path.parent.mkdir(parents=True, exist_ok=True)
                environment = os.environ.copy()
                environment.update({"CUDA_VISIBLE_DEVICES": str(gpu), "OPENBLAS_NUM_THREADS": "1", "OMP_NUM_THREADS": "1", "MKL_NUM_THREADS": "1", "NUMEXPR_NUM_THREADS": "1", "MPLCONFIGDIR": MPL_CACHE})
