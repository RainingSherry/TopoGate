#!/usr/bin/env python3
"""Multi-seed verification for LearnableGate (the current mainline).

Runs static_gate_full + learnable_gate_sched on 5 datasets × 3 seeds = 30 runs.
All results go to result/learnable_gate_smoke/multiseed/.
"""
...
def run_one(dataset: str, variant_name: str, overrides: dict, seed: int, gpu: int) -> dict:
    ...
    if not npz.exists():
        return {"dataset": dataset, "variant": variant_name, "seed": seed, "error": f"{npz} not found"}
