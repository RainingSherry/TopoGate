...
def _tie_key(ii: np.ndarray, jj: np.ndarray, *, seed: int, row: int) -> np.ndarray:
    # uint64 arithmetic gives a stable seed/row-specific order without random
    # state or a materialized random matrix.
    return (
        ii.astype(np.uint64) * np.uint64(1315423911)
        + jj.astype(np.uint64) * np.uint64(2654435761)
        + np.uint64(int(seed) * 97531 + int(row) * 433494437)
    )
