#!/usr/bin/env python3
"""Part B ablation for v9 = LearnableGate + Adaptive PCA.

Tests 4 ablation levels × 15 datasets × 3 seeds = 180 runs.

Ablation axis:
  L1 (v9_static_gate):       gate_mode=topology (static β)   | topo neighbours | mix_mode=reliability
  L2 (v9_random_neighbors):  gate_mode=learned               | random neighbours | mix_mode=random
  L3 (v9_static_random):     gate_mode=topology (static β)   | random neighbours | mix_mode=random
  L4 (v9_nomix):             gate_mode=learned               | mix_mode=none (no pseudo branch)

The v9 baseline (v9_adaptive) is NOT run here — it lives in run_v9_multiseed.py.

Outputs:
  result/v9_learnable_gate/ablation/<dataset>__<variant>__seed{N}.json
  result/v9_learnable_gate/ablation/comparison.csv
"""
...
# Note: L1/L3 use gate_mode=topology (static β at v1 defaults) → reads the static
# gate value, which inherits init β from args (beta_mutual=1, beta_snn=1,
# beta_perturb=2, beta_uncertainty=1).  This is exactly the static_gate_full
# behaviour, scoped to the v9 HVF+Adaptive-PCA frame.
VARIANTS = {
    "v9_static_gate": {
            ...
            for seed in seeds:
                ...
                if res.get("error"):
                    ...
                    print(
                        ...
                        f"K={res['n_clusters']}  time={res['elapsed']:.1f}s  ({time.time()-t_start:.1f}s wall)",
                        flush=True,
                    )
                rows.append(res)
