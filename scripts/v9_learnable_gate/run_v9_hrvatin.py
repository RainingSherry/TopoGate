#!/usr/bin/env python3
"""Run v9 (LearnableGate + Adaptive PCA) on hrvatin_filtered with HVF2000 + dim=500
to avoid the slow Adaptive-PCA-with-cap-2000 path.

This is the v9 variant that successfully ran on the 14 other datasets, but
configured for hrvatin's d=25187 by capping adaptive PCA at 500.
"""
...
def main():
    ...
    variant_name = "v9_adaptive_hrvatin"
        ...
        if json_path.exists():
            try:
                ...
                if d.get("ari") is not None and d.get("error") is None:
                    print(f"hrvatin  seed={seed}: cached ARI={d['ari']:.4f}, skip")
                    continue
            except Exception:
                pass
        print(f"hrvatin  seed={seed}", flush=True)
        t0 = time.time()
        try:
